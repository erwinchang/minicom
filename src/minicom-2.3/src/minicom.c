...
static ELM *getline_minic(WIN *w, int no)
{
  int i;
...
static void drawhist(WIN *w, int y, int r)
{
  ...
  w->direct = 0;
  for (f = 0; f < w->ys; f++)
    mc_wdrawelm(w, f, getline_minic(w, y++));
  if (r)
    mc_wredraw(w, 1);
  w->direct = 1;
}
              ...
              int case_matters) /* guess... */
{
  ...
  for (next_line = hit_line; next_line <= all_lines; next_line++) {
    ...
    tmp_e = getline_minic(w_hist, next_line);
...
static void drawcite(WIN *w, int y, int citey, int start, int end)
{
  if (y+citey >= start && y+citey <= end)
    mc_wdrawelm_inverse(w, y, getline_minic(w, y+citey));
  else
    mc_wdrawelm(w, y, getline_minic(w, y+citey));
}
  ...
  while (loop) {
    ...
    switch (c) {
      ...
      case K_UP:
        ...
        if (inverse)
          mc_wdrawelm_inverse(b_us, 0, getline_minic(b_us, y));
        else
          mc_wdrawelm(b_us, 0, getline_minic(b_us, y));
        if (citemode)
          mc_wlocate(b_us, 0, cite_y);
        mc_wflush();
        break;
      ...
      case K_DN:
        ...
        if (citemode) {
          ...
          tmp_e = getline_minic(b_us, y + b_us->ys - 1);
          if (wcslen(look_for) > 1) {
            /* quick scan for pattern match */
            mc_wdrawelm_var(b_us, tmp_e, tmp_line);
            inverse = (wcslen(tmp_line)>1 &&
        ...
        if (inverse)
          mc_wdrawelm_inverse(b_us, b_us->ys - 1,
                           getline_minic(b_us, y + b_us->ys - 1));
        else
      ...
      case 10: case 13:
        ...
        mc_wprintf(b_st, hline);
        mc_wredraw(b_st, 1);
        mc_wdrawelm_inverse(b_us, cite_y, getline_minic(b_us, cite_ystart));
        mc_wlocate(b_us, 0, cite_y);
        break;
      case K_ESC:
        if (!citemode) {
...
int main(int argc, char **argv)
{
  ...
  if (real_uid != 0 && real_uid != eff_uid) {
    ...
    if ((fp = fopen(userfile, "r")) != (FILE *)0) {
      while (fgets(buf, 70, fp) != NULL && !userok) {
        /* Read first word */
        bufp = buf;
        s = getword(&bufp);
      ...
      if (!userok) {
        ...
        fprintf(stderr,
                _("Ask your sysadmin to add your name to %s\n"),
                userfile);
        exit(1);
      }
    }
